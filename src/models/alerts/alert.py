    ...
    def send(self):
        return requests.post(
            ...
            data={
                ...
                "to": self.user_email,
                "subject": "Price limit reached fro {}".format(self.item.name),
                "text": "We've found the deal!({}).".format(self.item.url)
            }
        )

    @classmethod
    def find_needing_update(cls, mimutes_since_update=AlertConstants.ALERT_TIMEOUT):
        last_updated_limit = datetime.datetime.utcnow() - datetime.timedelta(minutes=mimutes_since_update)
